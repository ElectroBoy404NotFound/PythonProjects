...
def openFile():
    ...
    fileTextFile = filedialog.askopenfilename(title="Open File in PyPad++", filetypes=(("Text Files", "*.txt")))
    strFileName = os.path.basename(fileTextFile)
    strFormatedName = strFileName.split('.')[0]
    entryFileName.insert(END, strFormatedName)
    window.title("PyPad++ - " + strFormatedName)
    fileInputFile = open(strFileName, 'r')
...
buttonSave = Button(master=window, image=imageSave, text="Save File", command=None)
